                ...
                <div class="card-body" style="padding-right: 5px; height: 700px;">
                    ...
                    <div>
                        <div style="float: left;width: 100%;background-color: rgb(249,249,249);margin-top: -10px; border-bottom: 1px solid rgba(0,0,0,0.05); margin-bottom: 1px;">
                            ...
                            <div style="text-align: left; width: 100%; float: left; margin-right: 10px; border-right: 1px solid rgba(0,0,0,0.1)">
                            ...
                            <div style="float: left;">
                                <label style="float: left; margin-top: 5px; margin-right: 8px; margin-bottom: 0px;">z-value:</label>
                                <div :style="{'float':'left', 'margin-top':'5px'}">
                                    <slider :min=-0.000000001 :max=100 v-model = "ROI_z_value" :style="{'width': '100px'}"></slider>
                                </div>
                            </div>
                            <div style="float: left;">
                                <label style="float: left; margin-top: 5px; margin-right: 8px; margin-bottom: 0px;">DBScan radius:</label>
                                <div :style="{'float':'left', 'margin-top':'5px'}">
                                    <slider :min=-0.000000001 :max=100 v-model = "DBScan_radius" :style="{'width': '50px'}"></slider>
                                </div>
                            </div>
                            </div>
                        </div>
                        <div>
                            <div v-for="feature_value in feature_values" >
                                <div style="min-width: 20%;margin: auto; float: left;text-align: center;padding-bottom: 10px;">
                                    <div style="border-right: 1px solid rgba(0,0,0,0.05); border-bottom: 1px solid rgba(0,0,0,0.05);">
                                        ...
                                        <div style="min-height: 230px;">
                                            <superposed-graph-version-two v-bind="{'strength':features[feature_value].data}" :index='feature_value' :heightPer='0.2'></superposed-graph-version-two>
                                        </div>
                                        <button class="btn btn-secondary" @click="duplicate(feature_value, 0)" :style="{'width': '34px','position':'absolute', 'top':'311px', 'z-index':'999'}">
                                            +
                                        </button>
                                        <div v-if="duplicateData[feature_value].length > 0" style="height: 100px; max-width: 185px;">
                                            <div v-for="(dupli, dupliIndex) in duplicateData[feature_value]" style="float: left;">
                                                <superposed-graph-for-dupli v-bind="{'strength':dupli.data}" :index='feature_value' :heightPer='0.05'></superposed-graph-for-dupli>
                                                <button class="btn btn-secondary" @click="deleteDuplicate(feature_value, 0, dupliIndex)" :style="{'width': '34px'}">
                                                    -
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <div style="border-right: 1px solid rgba(0,0,0,0.05); padding-bottom: 20px;">
                                        <div style="float: left; z-index: 999; width: 95px; text-align: left; padding-left: 5px;">
                                            ...
                                            <Label style="width: 70px">multi-{{features[feature_value].name}}</Label>
                                        </div>
                                        <superposed-graph-for-multi-version-two v-bind="{'strength':features[feature_value].data}" :index='feature_value' :heightPer='0.2'></superposed-graph-for-multi-version-two>
                                        <button class="btn btn-secondary" @click="duplicate(feature_value, 1)" :style="{'width': '34px','position':'absolute', 'top':'511px', 'z-index':'999'}">
                                            +
                                        </button>
                                        <div v-if="duplicateData[Number(feature_value) + 5].length > 0" style="height: 100px; max-width: 185px;">
                                            <div v-for="(dupli, dupliIndex) in duplicateData[Number(feature_value) + 5]" style="float: left;">
                                                <superposed-graph-for-multi-dupli v-bind="{'strength':dupli.data}" :index='feature_value + 5' :heightPer='0.05'></superposed-graph-for-multi-dupli>
                                                <button class="btn btn-secondary" @click="deleteDuplicate(feature_value, 1, dupliIndex)" :style="{'width': '34px'}">
                                                    -
                                                </button>
                                            </div>
                                        </div>
...
<script>
    ...
    export default {
        name: "comparisonViewVersionThree",
        components: {
            superposedGraph,
        ...
        watch: {
            lambda: {
                ...
                handler: function(newVal, oldVal) {
                    ...
                    NetService.update2D(this.pValueThreshold, this.validThreshold, this.testVal,this.lambda, this.ROI_z_value, this.DBScan_radius, (res, err) => {
                        if(err){
                            console.error(err);
                            return;
                        }
            ...
            ROI_z_value: {
                deep: true,
                handler: function(newVal, oldVal) {
                    console.log("ROI_z_value change: " + newVal);
            ...
            DBScan_radius: {
                deep: true,
                handler: function(newVal, oldVal) {
                    console.log("DBScan_radius change: " + newVal);
            ...
            multiVal: {
                deep: true,
                handler: function(newVal, oldVal) {
                    this.$store.commit('set_features_data_multidata', newVal);
            ...
            testVal: {
                ...
                handler: function(newVal, oldVal) {
                    ...
                    NetService.update2D(this.pValueThreshold, this.validThreshold, newVal, this.lambda, this.ROI_z_value, this.DBScan_radius, (res, err) => {
                            if(err){
                                console.error(err);
                                return;
                            }
