<template>
	<div style="height: 100%">
        <div style="padding: 0px;width: 1202.5px; float: left; height: 100%;">
            <div style="height: 100%;">
                <div class="mycard" style="padding-right: 5px; height: 100%; margin-left: 30px; margin-right: 0px;">
                    ...
                    <div>
                        <div class="mypannel" style="float: left;width: 100%; margin-top: -10px;margin-bottom: 1px; font-size: 5px; margin-left: -5px;">
                            ...
                            <ui-select
                                ...
                                style="text-align: left; float: left;"
                            ></ui-select>
                            <span style="float: left; font-size: 14px; padding-top: 1px; margin-left: 7px; margin-right: -6px;">Univariate - </span>
                            <ui-select
                                label="Edge Stat.: "
                                name="linksPros"
                                :options="linksPros"
                            ...
                            <ui-select
                                ...
                                style="text-align: left; float: left;"
                            ></ui-select>
                            <span style="float: left; font-size: 14px; padding-top: 1px; margin-left: 7px; margin-right: -6px;">Multivariate - </span>
                            <ui-select
                                label="Edge Stat.: "
                                name="multiValPros"
                        ...
                        <div style="height: 575px; width: 1150px;">
                            <happy-scroll color="rgba(0,0,0,0.5)" resize>
                                <div :style="{'width': (/*feature_values.length + */(Math.max(singlefeatures.length, multifeatures.length)) * 226 + 20).toString() + 'px', 'height': '570px'}">
                                    <vuedraggable :style="{'float':'left', 'width': Math.max(singlefeatures.length, multifeatures.length) * 226 + 20 + 'px'}" v-model="singlefeatures">
                                        <transition-group>
                                            <div v-for="(feature, index) in singlefeatures" :key="index">
                                                <div style="width: 226px; height: 555px;margin: auto; float: left;text-align: center;padding-bottom: 0px; z-index: 0;" :class="{'mySelected': index===selected_index, 'unSelected': index != selected_index}">
                                                    ...
                                                    <div style="border-bottom: 1px solid rgba(0,0,0,0.05); position: relative;">
                                                        <loading v-bind="{'loading': feature.singleLoading}"></loading>
                                                        <button class="mydeletebtn" @click="deleteSingle(index)" :style="{'position': 'absolute', 'top':'5px', 'left': '206px', 'z-index': 10}">
                                                            ×
                                                        </button>
                                                        <div style="position: absolute; z-index: 5; width: 100%; text-align: center; padding-left: 5px;">
                                                            <div>
                                                        ...
                                                        <div style="min-height: 275px;" @click="changeSelected(index)">
                                                            <div style="position: absolute; left: 177px; top: 244px;">
                                                            ...
                                                            <superposed-graph-version-five v-bind="{'strength':singlefeatures[index].data, 'filterdata':singlefeatures[index].filterSingleData, 'sidebyside':singlefeatures[index].sameFunc, 'sidebysideMax':singlefeatures[index].sidebysideMax, 'sidebysideMin':singlefeatures[index].sidebysideMin, 'sidebysideTrueMax':singlefeatures[index].sidebysideTrueMax, 'Func': singlefeatures[index].singleFunc, 'eyeside':singlefeatures[index].eyeside, 'g1name': singlefeatures[index].g1name, 'g2name': singlefeatures[index].g2name, 'selected': selectValue.indexOf((index * 2).toString()) != -1,'filterRate': feature.singlefilterRate, 'filterdataLen':feature.filtersingledataLen, 'showFunction':feature.showFunction, 'algorithm':feature.testValName}" :index='index.toString()' :heightPer='0.2' :featureName='feature.name'></superposed-graph-version-five>
                                                        </div>
                                                    </div>
                                                    <div style="position: relative;">
                                                        <loading v-bind="{'loading': feature.showFunction == 0 ? feature.multiLoading : feature.singleLoading}"></loading>
                                                        <div v-show="feature.message != 'Success' && feature.showFunction == 0">
                                                            <div class="message">{{feature.message}}</div>
                                                        </div>
                                                        <div style="position: absolute; z-index: 999; width: 100%; text-align: center; padding-left: 5px;">
                                                            <div>
                                                        ...
                                                        <div style="min-height: 275px;" @click="changeSelected(index)">
                                                            ...
                                                            <superposed-graph-for-multi-version-four v-bind="{'strength':singlefeatures[index].data, 'filterdata':singlefeatures[index].filterMultiData, 'sidebyside':singlefeatures[index].sameFunc, 'sidebysideMax':singlefeatures[index].sidebysideMax, 'sidebysideMin':singlefeatures[index].sidebysideMin, 'sidebysideTrueMax':singlefeatures[index].sidebysideTrueMax, 'Func': singlefeatures[index].multiFunc, 'eyeside':singlefeatures[index].eyeside, 'g1name': singlefeatures[index].g1name, 'g2name': singlefeatures[index].g2name, 'selected': selectValue.indexOf((index * 2 + 1).toString()) != -1, 'filterRate': feature.sync.length > 0 ? feature.singlefilterRate : feature.multifilterRate, 'filterdataLen':feature.filtermultidataLen, 'showFunction':feature.showFunction, 'lambda':feature.lambda[0], 'lambdaMax':feature.maxLambda, 'lambdaMin':feature.minLambda, 'algorithm':feature.multiGroupValName}" :index='index.toString()' :heightPer='0.2' :featureName='feature.name'></superposed-graph-for-multi-version-four>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
        ...
        <div style="padding: 0px;width: 420.5px;float: left; height: 100%;">
            ...
            -->
                ...
                <div style="float: left; width: 100%; margin-top: 0px; margin-left: 0px;">
                    <superposed-graph-for-cal-two-with-ellipse-version-three v-bind="{'strength':totalData, 'filterRate':filterRate, 'collection': totalData.collection}" :heightPer='0.4' :index="String(10)"></superposed-graph-for-cal-two-with-ellipse-version-three>
...
<script>
    ...
    import superposedGraphForCalTwoWithEllipseVersionThree from "./superposedGraphForCalTwoWithEllipseVersionThree"
    import vuedraggable from 'vuedraggable'
    import { UiAutocomplete, UiCollapsible, UiAlert, UiSelect, UiButton, UiDatepicker, UiRadioGroup} from 'keen-ui'
    import loading from "./loading"

    export default {
        name: "comparisonViewVersionFive",
        components: {
            superposedGraph,
            ...
            UiRadioGroup,
            loading,
            superposedGraphForCalTwoWithEllipseVersionThree
        },
        data () {
            return {
                strength,
        ...
        computed: {
            ...mapState(['pValueThreshold', 'validThreshold', 'features', 'localFeature', 'feature_values', 'multifeatures', 'selected_index', 'feature_index', 'needReDraw', 'dmax']),
            singlefeatures: {
                get() {
            ...
            testVal: {
                ...
                set(newVal) {
                    ...
                    if(this.singlefeatures.length > 0) {
                        ...
                        NetService.singleAnalyzeForOnefeature(index, this.pValueThreshold[Number(index)], this.validThreshold[Number(index)], this.testVal.index, this.singlefeatures[localindex].groupName, this.singlefeatures[localindex].subjectList, (res, err) => {
                            if(err){
                                console.error(err);
                                return;
                            }
            ...
            multiGroupVal: {
                ...
                set(newVal) {
                    ...
                    if(this.singlefeatures.length > 0) {
                        ...
                        NetService.multiAnalyzeForOnefeature(index, this.pValueThreshold[Number(index)], this.validThreshold[Number(index)], this.$store.state.multiGroupVal, this.$store.state.ROIZValue, this.$store.state.DBScanRadius, -1, this.singlefeatures[localindex].groupName, this.singlefeatures[localindex].subjectList, (res, err) => {
                            if(err){
                                console.error(err);
                                return;
                            }
            ...
            modelVal: {
                get() {
                    return this.modelValProsl[Number(this.$store.state.modelVal)];
                },
                set(newVal) {
                    this.$store.commit('set_modelVal', newVal.index);
                    if(this.singlefeatures.length > 0) {
                        var localindex = this.selected_index;
                        var index = this.feature_index;
                        //添加多变量
        ...
        watch: {
            ...
            totalData: {
                ...
                handler: function(newVal, oldVal) {
                    if(newVal && this.totalData.allDatas.length > 0){
                        //获取新的分布
                        NetService.getCirclePoint(this.totalData.data, this.totalData.allDatas.length, this.dmax, (res, err) => {
                            if(err){
                                console.error(err);
                                return 0;
                            }
                            ...
                            this.pointPos = res.data.pointPos;
                        });
                    }
                }
            },
