<template>
  <div class="graph">
    ...
    <div :style="{'text-align':'left', 'height':'0px', 'position':'absolute', 'right':'25px', 'top':'26px', 'font-size':'10px'}">
      ...
      */
      ...
      this.vis
        ...
        .attr("transform", function(d, index) {
          var result =  "rotate(" +　(index==0 ? d.angle * 180 / Math.PI : d.angle * 180 / Math.PI - 10) + ")";
          var sX = index==0 ? 3 : 0;
          var sY = index==0 ? 0 : (-1 * (Math.max(0, scaleY(d.y1))) * that.width  * that.datadepth / 4 / 2 + 12); //+ 5
          result += "translate(" +  sX +"," + sY + ")";
          //if (d.angle >= Math.PI * 3 / 4 && d.angle <= Math.PI * 5 / 4 )
          if (d.angle == Math.PI)
            result += "rotate(180)";
