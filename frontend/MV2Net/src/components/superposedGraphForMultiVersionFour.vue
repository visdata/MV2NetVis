<!--叠加图，strength从外部绑定 正负multisign,粗细multidata，展示数目多少是strength.selectConnection-->
<template>
    <div style="text-align: left; padding-bottom: 5px;">
        <div :class='setClass()' style="height:50%;">
            <loading v-bind="{'loading': load}"></loading>
            <div style="float: left; z-index: 999; width: 100%; text-align: center; padding-left: 5px; border-bottom: 1px solid rgba(0,0,0,0.05);">
                <div style="padding-left: 16px;">
                    <Label style="padding-right: 0px; color: white;">{{showFunction == 0? ("Multivariate view:" ): (g2name + " Group:")}}</Label>
            ...
            <div style="position: absolute; top: 30px; padding-left: 4px; z-index: 9;">
                ...
                <div>
                    ...
                    <span style="vertical-align: super; margin-left: 2px;">
                        {{sidebyside ? (Func == "p-value" ? g1name : g2name) : g1name}}
                    </span>
                </div>
            ...
            <!--div style="position: absolute; top: 512px; padding-left: 10px; z-index: 999;">
                <div>
            ...
            <div class="tipsDiv" style="z-index: 10; color: black; font-size: 12px; text-align: left;position: absolute;visibility: hidden; height: 0px;"></div>
            <div class="leftSVG" style="padding-left: 20px;padding-top: 0px; min-height: 241px;">
                
            </div>
            <div class="panel" ref="myPanel" style="overflow: visible; font-size: 5px; width: auto; float: right;">
                <div style="float: left; margin-top: -11px; width: auto; text-align: center; font-size: 14px;padding-right: 8px; line-height: 16px;" v-if="showFunction == 1">{{algorithm}}<br>{{Func ? (Func + ":") : ""}}</div>
                <div style="width: 95px;float: left;" v-if="showFunction == 1">
                ...
                <div style="float: left; margin-top: -11px; width: auto; text-align: center; font-size: 14px;padding-right: 8px; line-height: 16px;" v-if="showFunction == 0">{{algorithm}}<br>Lambda:</div>
                <div style="width: 95px;float: left;" v-if="showFunction == 0">
                    <slider :min=0 :max=100 v-model="sliderLambda" :dataMin="-lambdaMin" :dataMax="-lambdaMax" :dataTrueMax="-lambdaMax" :ispvalue=1 :style="{'width': '95px'}"></slider>
                </div>
                <div v-if="showFunction == 0" style="float: left; margin-top: -3px; width: auto; text-align: right; font-size: 12px;padding-left: 5px;padding-right: 3px;">Acc:{{accuracy.toFixed(2)}}</div>
                <div v-if="showFunction == 1" style="float: left; margin-top: -3px; width: 51.44px; text-align: right; font-size: 12px;padding-left: 5px;padding-right: 3px;"></div>
                <!--div>
                    <div style="float: left; width: 40%; text-align: left;">
                        <span>{{thick_range.r1.toFixed(2)}}</span>
                    </div>
                    <div style="float: left; width: 60%; text-align: right;">
                        <span>{{thick_range.r2.toFixed(2)}}</span>
                    </div>
                </div-->
            </div>
...
<script>
    ...
    //import $ from "jquery"
    export default {
        name: 'superposedGraphForMultiVersionFour',
        components: {
            slider,
            reverseSlider,
            loading
        ...
        data() {
            return {
                ...
                isFocus: false,
                //filterRate: [0, 100],
                //lambdaMin: Math.log(0.0001),
                //lambdaMax: Math.log(10),
                //lambda: 0,
                weight_range: {
                    'r1' : 0,
                    'r2' : 0,
        ...
        computed: {
            ...mapState(['pValueThreshold', 'validThreshold', 'selected_index', 'feature_index', 'singlefeatures']),
        },
        //mounted在html渲染出来后进行操作
        mounted() {
            if(this.strength.hasOwnProperty('multidata'))
                this.drawSidBySidePic();
            this.sliderLambda = this.lambda;
        },
        methods: {
            changePurple() {
            ...
            drawSidBySidePic(){
                ...
                */
                ...
                //读取weight，thickness
                this.accuracy = this.strength.accuracy;
                for(var i = 0; i < this.strength.multidata.length; i++) {
                    indexWeight[i] = this.strength.selectConnection[i];
                    indexThickness[i] = Math.abs(this.strength.multidata[i]);
                    sign[i] = this.strength.multisign[i];
                ...
                if(that.filterRate[0] == 0) {
                    ...
                    that.matrix.links.forEach(function(d,i) {
                        if (d.target == d.source) {
                            ...
                            if(d.thick > 0 && d.weight > 0 && ((that.showPurple && d.sign >= 0) || (that.showOrange && d.sign < 0)) ) {
                                if(d.sign >= 0)
                    ...
                    /*
                    for(var i = 0; i < this.strength.multidata.length; i++) {
                        if(this.strength.selectConnection[i] > 0)
                            this.filterdata[i] = this.strength.multidata[i];
                        else 
                            this.filterdata[i] = 0;
                    }
                    */
                    ...
                    that.matrix.links.forEach(function(d,i) {
                        if (d.target == d.source) {
                            ...
                            if(d.thick >= fillMin && d.weight > 0&& ((that.showPurple && d.sign >= 0) || (that.showOrange && d.sign < 0))) {
                                if(d.sign >= 0)
                                    that.filterdata[i] = d.thick;
                                else that.filterdata[i] = -d.thick;
                                countfilter++;
                    ...
                    /*
                    for(var i = 0; i < this.strength.multidata.length; i++) {
                        if (Math.abs(this.strength.data[i]) >= fillMin && this.strength.selectConnection[i] > 0) {
                            this.filterdata[i] = this.strength.multidata[i];
                            //console.log("this.filterdata", this.filterdata[i] );
                        }
                        else {
                            this.filterdata[i] = 0;
                        }
                ...
                */
                ...
                var linkLeft = svgLeft.selectAll(".link")
                    ...
                    .style("stroke", function(d){//路径颜色
                        ...
                        if(Math.abs(d.thick) >= fillMin && Math.abs(d.thick) <= fillMax){
                            if(d.sign >= 0){
                                return that.fillScaleOne(Math.abs(d.thick) - fillMin, fillMax - fillMin);
                            } else {
                                return that.fillScaleTwo(Math.abs(d.thick) - fillMin, fillMax - fillMin);
                            }
                        } else {
                ...
                var linkLeft2 = svgLeft.selectAll(".link2")
                    ...
                    .on("mouseenter", function(d, i) {
                        ...
                        return tipsDiv
                            ...
                            //.style('top', (d3.event.clientY - 390) + 'px')
                            //.style('left', '10px')
                            //.style('top', '224px')
                            ...
                            .html('ROI: ' + (d.source.index + 1) + '  <->  ' + (d.target.index + 1) + '<br>' + that.Func + ': ' + (that.Func == "p-value" ? d.origindata.toFixed(5) : d.thick.toFixed(5)) + '<br>');//tooltip.html(text);
                    })
                ...
                nodeLeft.append("circle")
                    ...
                    .on("mouseenter", function(d, i) {
                        ...
                        that.matrix.links.filter(function(d1){
                            return d1.weight > 0 && d1.thick >= fmin && d1.thick != 0 && ((that.showPurple && d1.sign >= 0) || (that.showOrange && d1.sign < 0)) && (d1.target.index == d.index || d1.source.index == d.index);
                        }).forEach(function(d1){
                            nodelinknum++;
                        });
        ...
        watch: {
            ...
            sliderLambda: {
                ...
                handler: function(newVal, oldVal) {
                    console.log("oldVal", oldVal);
                    if(oldVal != -1 && this.sliderChange) {
                        var localindex = this.index;
                        var index = this.$store.state.singlefeatures[localindex].index;
                        this.load = true;
                        NetService.multiAnalyzeForOnefeature(index, this.pValueThreshold[Number(index)], this.validThreshold[Number(index)], this.$store.state.singlefeatures[localindex].multiGroupVal, this.$store.state.ROIZValue, this.$store.state.DBScanRadius, Math.pow(Math.E,this.sliderLambda / 100 * (this.lambdaMax - this.lambdaMin) + this.lambdaMin), this.singlefeatures[localindex].groupName, this.singlefeatures[localindex].subjectList, (res, err) => {
                            if(err){
                                console.error(err);
                                return;
                            }
                            ...
                            console.log('multiAnalyzeForOnefeature for lambda', res.data, localindex);
                            ...
                            this.$store.commit('set_selectedFeature_data_multidata', [Number(this.$store.state.multiVal), localindex]);
                            //this.drawSidBySidePic();
                            this.load = false;
                        });
                    }
            ...
            strength: {
                ...
                handler: function(newVal, oldVal) {
                    if(newVal){
                        console.log('drawSidBySidePic', newVal);
                        if(this.strength.hasOwnProperty('multidata'))
                            this.drawSidBySidePic();
                    }
                }
                ...
                handler: function(newVal, oldVal) {
                    ...
                    */
                    ...
                    svgLeft.selectAll(".link").filter(function(d){
                        return d.weight > 0 && d.thick >= fmin && d.thick != 0 && ((that.showPurple && d.sign >= 0) || (that.showOrange && d.sign < 0));
                ...
                handler: function(newVal, oldVal) {
                   // console.log("reverse", newVal);
                    ...
                    svgLeft.selectAll(".link")
                        .style("stroke-width", function(d) {
                            if (newVal)
                                return 1; //(Math.abs(d.thick) - that.thick_range.r1) / (that.thick_range.r2 - that.thick_range.r1) * 3 + 1;
                            else return 4 - (Math.abs(d.thick) - that.thick_range.r1) / (that.thick_range.r2 - that.thick_range.r1) * 4 
                        });
                }
            },
