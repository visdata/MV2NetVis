<template>
    ...
    <div :style="{'margin-top': '-10px', 'position':'absolute', 'padding-left': '70%', 'z-index': '99'}">
        <ul class="ui-controlgroup ui-listview" :style="{'list-style-type':'none', 'margin':'1px', 'padding':'0px', 'text-align':'left'}">
            <li v-for="(item, index) in pro_selections_items" :key="item.index">
                ...
                <input class="custom-control-input" type="checkbox" @click="multipleSelect(item, item.checkval)" :checked="item.checkval" />
    ...
    <div :style="{'min-height': '56px', 'margin-top': '-8px'}">
        <div :style="{'display':'inline', 'width':'150px', 'float':'left'}">
            ...
            <slider :min=-0.000000001 :max=100 v-model = "pValueThreshold" :style="{'width': '150px'}"></slider>
        ...
        <div :style="{'display':'inline', 'width':'180px', 'float':'left','padding-left':'30px'}">
            ...
            <slider :min=-0.000000001 :max=100 v-model = "validThreshold" :style="{'width': '150px'}"></slider>
    ...
    <!--第一列下热力图-->
    <div :style="{'height': '270px', 'width': '300px', 'padding-top':'30px'}">
        ...
        <happy-scroll color="rgba(0,0,0,0.5)" size="5">
            <div class="featureView" :style="{'text-align': 'left'}" >
                <div v-for="feature in features" :key="feature.index" style="margin-bottom:10px">
                    ...
                    <chart :options="feature.options" :auto-resize='false' :style="{width: '300px', height: '270px', margin: 'auto'}"></chart>
                </div>
            </div>  
        </happy-scroll>
    </div>
...
<script>
    ...
    export default {
        name: "echartsScatterVersionFive",
        components: {
            breadcrumbTrail,
            highlightOnHover,
            nodeInfoDisplayer,
        ...
        computed: {
            ...mapState(['linksVal', 'testVal', 'features', 'subjects_valid', 'selected_subjects', 'selected_subjects_name']),
        },
        methods: {
            sliderFun(){
            ...
            validThreshold: {
                ...
                handler: function(newVal, oldVal) {
                    if(newVal){
                        ...
                        NetService.update2D(this.pValueThreshold, this.validThreshold, this.testVal, (res, err) => {
                            if(err){
                                console.error(err);
                                return;
                            }
