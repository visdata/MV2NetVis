...
# fn1为特征数据文件，fn2为用户信息文件
fn1 = 'selected_dict.json'
fn2 = 'subject_all_198.txt'
feature_name = ['connectivity', 'length', 'entropy', 'curvature', 'torsion']

# 已废弃
def divideSets(dataSet, arrIndex):
	p1 = []
	p2 = []
	for x in range(len(dataSet)):
		if int(dataSet[arrIndex]) > 0:
			p1.append(dataSet[arrIndex])
		else:
			p2.append(dataSet[arrIndex])
	return p1, p2

# 已废弃
def getSunBurstData(subject_list, propertiesArr=[1,2]):
	firstLayer1, firstLayer2 = divideSets(subject_list, propertiesArr[0])
	secondLayer1, secondLayer2 = divideSets(firstLayer1, propertiesArr[1])
	secondLayer3, secondLayer4 = divideSets(firstLayer2, propertiesArr[1])
	return secondLayer1, secondLayer2, secondLayer3, secondLayer4
	
# 已废弃
def getData():
	selected_dict = {}
	with open(fn1, 'r') as ifile:
		selected_dict = json.load(ifile)
	subject_list = [x.rstrip().split('\t') for x in open(fn2, 'r')]
	keys = list(selected_dict.keys())
	post_dict = {}
	post_dict['code'] = 0
	data = [["balance","subjects","pvalue","featurename"]]
	for x in range(len(subject_list)):
		line = [int(random.uniform(0,10)), x, selected_dict[keys[x]], feature_name[int(int(keys[x])%5)]]
		data.append(line)
	post_dict['data'] = data
	femaleAD, femaleHealthy, maleAD, maleHealthy = getNum(subject_list)
	post_dict['femaleAD'] = femaleAD
	post_dict['femaleHealthy'] = femaleHealthy
	post_dict['maleAD'] = maleAD
	post_dict['maleHealthy'] = maleHealthy
	post_dict['message'] = 'Success'
	return post_dict

# 根据用户属性返回数据
def getDataByPro(properties):
	selected_dict = {}
	with open(fn1, 'r') as ifile:
		selected_dict = json.load(ifile)
	subject_list = [x.rstrip().split('\t') for x in open(fn2, 'r')]
...
# 根据用户属性和特征名返回数据
def getDataByPF(properties, featurename):
	selected_dict = {}
	with open(fn1, 'r') as ifile:
		selected_dict = json.load(ifile)
	subject_list = [x.rstrip().split('\t') for x in open(fn2, 'r')]
	...
	data = [["balance","subjects","pvalue","featurename"]]
	for x in range(len(subject_list)):
		if featurename == feature_name[int(int(keys[x])%5)]:
			line = [int(random.uniform(0,10)), x, selected_dict[keys[x]], feature_name[int(int(keys[x])%5)]]
			data.append(line)
	post_dict['data'] = data
	femaleAD, femaleHealthy, maleAD, maleHealthy = getNum(subject_list)
...
# 根据用户属性更新sunburst
def updateSunburst(propertiesArr):
	# print(propertiesArr)
	res_dict = {}
...
# 根据用户组返回p-value
def getPValue(subjectsArr):
	return {}

# 根据用户组返回p-value
def getPValueSingle(groupid, subjectsArr):
	start_time = time.time()
	print(subjectsArr)
	# subjectsArr = json.loads(subjectsArr)
	SUBJECTS = ["", ""]
	SUBJECTS[int(groupid)-1] = subjectsArr
	# subjectsArr=["all-healthy-female","all-healthy-male"]
	normVal = []
	with open('normalValue_198.json', 'r') as ifile:
	...
	if int(groupid) == 1: 
		for i in g1_indices:
			...
			for j in range(int(len(nv)/5)):
				li = j*5
				# [strength, length, curvature,  trosion, entropy]
				line = [float(nv[li]),float(nv[li+1]),float(nv[li+3]),float(nv[li+4]),float(nv[li+2]),name,j]
				# line = [float(nv[li]),float(nv[li+1]),float(nv[li+2]),float(nv[li+3]),float(nv[li+4]),name]
				part.append(line)
...
# 已废弃
def getLassoScore(data, label, a):
	if len(data) == 0:
		return 0
	...
	# print(data, label)
	# score = cross_val_score(model, data, label, cv=3, scoring='accuracy')
	# score = score.mean()
	if math.isnan(score):
		score = 0
	return score
...
def filter_function(data, thes):
	...
	for idx in range(5*2485):
		...
		filted_f = [f[x] if selected_f[x] else 0 for x in range(87)]
...
# 更新comparison view
def updateTwoD(pvalueThreshold, validThreshold, testIndex, lamba, ROI_z_value, DBScan_radius):
	res_dict = {}
	return res_dict
	
	
# 单独返回feature view的pvalue
def getFeatureViewPValueSingle(featureIndex_, pvalueThreshold, validThreshold, testIndex, groupName_, subjectList_):
	start_time = time.time()
	pvalueThreshold = float(pvalueThreshold) / 100
	validThreshold = float(validThreshold) / 100
	featureIndex = int(featureIndex_)
	# featureIndex: 0=strength,1=length,2=curvature,3=torison,3=entropy
	if featureIndex == 0:
		featureIndex = 0
	elif featureIndex == 1:
		featureIndex = 1
	...
	data = []
	with open('all_feature_198.json', 'r') as ifile:
	...
	g1p = np.array([x.std() for x in featureArrG1[featureIndex] ])
	g2p = np.array([x.std() for x in featureArrG1[featureIndex] ])
	...
	g2p = g2p[not_self_connect_mask]
	
	print('G1P:',len(g1p[g1p!=0]))
	print('G2P:',len(g2p[g2p!=0]))
...
def getComparisonViewSingle(featureIndex_, pvalueThreshold, validThreshold, functionIndex, ROI_z_value, DBScan_radius, lambda_, groupName_, subjectList_):
	start_time = time.time()
	featureIndex = int(featureIndex_)
	# featureIndex: 0=strength,1=length,2=curvature,3=torison,3=entropy
	if featureIndex == 0:
	...
	name = ['strength', 'length', 'entropy', 'curvature', 'torsion']
	...
	for index in range(featureIndex, featureIndex+1):
		...
		if int(functionIndex) < 2:
			...
			with open('cache/feature_'+str(index)+str(ROI_z_value)+str(DBScan_radius)+SUBJECTS[0]+SUBJECTS[1]+'.json', 'w') as inputfile:
				json.dump(data_.tolist(), inputfile)
			with open('cache/label_'+str(index)+str(ROI_z_value)+str(DBScan_radius)+SUBJECTS[0]+SUBJECTS[1]+'.json', 'w') as inputfile:
				json.dump(label.tolist(), inputfile)
			with open('cache/index_'+str(index)+str(ROI_z_value)+str(DBScan_radius)+SUBJECTS[0]+SUBJECTS[1]+'.json', 'w') as inputfile:
				json.dump(group_indices_pack.tolist(), inputfile)
			if float(lambda_) == -1:
				proc = subprocess.Popen('Rscript grplasso.R '+str(index)+' '+str(ROI_z_value)+' '+str(DBScan_radius)+' '+SUBJECTS[0]+' '+SUBJECTS[1], shell=True)
				proc.wait()
				with open('cache/lambda_'+str(index)+str(ROI_z_value)+str(DBScan_radius)+SUBJECTS[0]+SUBJECTS[1]+'.json','r') as ifile:
					best_lambda = json.load(ifile)
					best_lambda = best_lambda[-1]
			else:
				proc = subprocess.Popen('Rscript grplasso_with_lambda.R '+str(index)+' '+str(ROI_z_value)+' '+str(DBScan_radius)+' '+SUBJECTS[0]+' '+SUBJECTS[1]+' '+lambda_, shell=True)
				proc.wait()
				best_lambda = float(lambda_)
			
			groups = group_indices_pack
			
			with open('cache/lass_coef_'+str(index)+str(ROI_z_value)+str(DBScan_radius)+SUBJECTS[0]+SUBJECTS[1]+'.json','r') as ifile:
...
def get_pt_line_dis(pt, line):
	...
	QP = np.array([pt[0] - line[0][0], pt[1] - line[0][1]])
	v = np.array([line[0][0] - line[1][0], line[0][1] - line[1][1]])
...
def cost_length(pt1, pt2, lmin, ALPHA, n):
	length = get_pts_dis(pt1, pt2)
	return 1/ALPHA*(length/lmin-n+1)
	'''
	if length >= lmin:
		return 1/ALPHA*(length/lmin-n+1)
	else:
...
def getDi(line, dmax, n, signal, R0, R1):
	...
	di = []
	# nmax = n
	nmax = int(length2 / lmin)
	for i in range(1,nmax+1):
		new_vec = np.array([line[0][0], line[0][1]]) + unit_vec*r + unit_vec*i*lmin
		di.append([new_vec[0], new_vec[1], 0])
	return di, lmin, notcover
...
def adjust_algorithm(totalData_, circleNum_, R1_, R0_, dmax_):
	...
	# totalData = totalData_
	
	for item in totalData:
		mask.append([float(item), float(item)])
	...
	for eIndex in range(len(edges)):
		...
		dis, lmin, notcover= getDi(e, dmax, n, signal, R0, R1)
		full.append(notcover)
			
		if len(dis) == 0 or signal[0] == 0:
			circlePoints.append([[0,0,0]]*n)
			continue
		cdis = []
		for di_ in dis:
			mindis = 999999999
			for e_index in range(len(edges)):
				e_ = edges[e_index]
				signal_ = mask[e_index]
				if signal_[0] != 0 and e_index != eIndex:
					tempdis = get_pt_line_dis(di_, e_)
					if tempdis < mindis:
						mindis = tempdis
			# 加入点到ROI圆的cost
			n_mindis = 999999999
			for n_index in range(70):
				if ROI_mask[n_index] == 0:
					continue
				node = nodes_pos[n_index]
				tempdis = get_pts_dis(di_, node)
				if tempdis < n_mindis:
					n_mindis = tempdis
			cdis.append(cost_dist(mindis, dmax)+cost_dist(n_mindis, R1+dmax))
		# Step2
		cost_all1 = 0
		cost_dist_all1 = 0
		case = 0
		...
		# cost_all1 = cost_dist_all1 + 1/ALPHA*(get_pts_dis(dis[0], dis[n-1])/lmin-n+1)
		cost_all1 = cost_dist_all1 + cost_length(dis[0], dis[n-1], lmin, ALPHA, n)
		# case2
		cost_all2 = 0
		cost_dist_all2 = 0
		...
		if len(pos) < n:
			...
			# cost_all2 = cost_dist_all2 + 1/ALPHA*(get_pts_dis(pos[0], pos[n-1])/lmin-n+1)
			cost_all2 = cost_dist_all2 + cost_length(pos[0], pos[n-1], lmin, ALPHA, n)
		# print(cost_all1, cost_all2)
		if cost_all1 <= cost_all2:
			# 选择第一种缺省方案
			case = 1
		else:
			# 选择第二种缺省方案
		...
		if case == 1:
			...
			for dIndex in range(n-1, len(dis)):
				...
				# temp_cost_all += temp_cost_all + cdis[dIndex] + 1/ALPHA*(get_pts_dis(dis[temp_selected_indices[0]], dis[temp_selected_indices[-1]])/lmin-n+1)
				temp_cost_all += temp_cost_all + cdis[dIndex] + cost_length(dis[temp_selected_indices[0]], dis[temp_selected_indices[-1]], lmin, ALPHA, n)
...
if __name__ == '__main__':
	'''
	temp_data = []
	with open('filted_fea_3.json', 'r') as ifile:
		temp_data = json.load(ifile)
	df = pd.DataFrame(temp_data)
	df.to_excel('filted_fea_3'+'.xlsx')
	'''
	subjectsArr = json.dumps(["all-AD","all-healthy"])
	res_dict = getPValue(subjectsArr)
	# res_dict = updateTwoD(50,50,0,5,21,50)
	with open('getPvalue.json', 'w') as ifile:
		json.dump(res_dict, ifile)
	# print(res_dict)
	'''
	normalValue = []
	with open('normalValue.json', 'r') as ifile:
		normalValue = json.load(ifile)
	normalValue = np.array(normalValue)
	selected_index = [True if x%5==0 else False for x in range(5*2485)]
	selected_index = np.array(selected_index)
	selected_normal = normalValue[:, selected_index]
	selected_normal = selected_normal.reshape(-1)
	count= [0, 0]
	for x in selected_normal:
		if x==1:
			count[0] += 1
		elif x>0.9 and x<1:
			count[1] += 1
	print(count[0]/len(selected_normal), count[1]/len(selected_normal))
	'''
